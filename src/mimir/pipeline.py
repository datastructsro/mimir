...
def run_pipeline(
    ...
    try:
        ...
        warehouse_id_lookup = {str(warehouse_id): warehouse_id for warehouse_id in warehouse_codes}
        ...
        minmax_table, warehouse_id = load_minmax_table(parquet_input_dir, config, warehouse_id_lookup)
        forecast_evidence = load_forecast_evidence_table(parquet_input_dir, config, warehouse_id)
...
def _build_imported_rules(
    *,
    ...
    planned_lead_time_days = lead_times_by_facility.get(str(warehouse_id), 0)
...
def _build_existing_orderpoint_keys(existing_orderpoints: pa.Table | None) -> set[tuple[int, int]]:
    ...
    return set(
        zip(
            existing_orderpoints.column("_odoo_product_id").to_pylist(),
            existing_orderpoints.column("_odoo_warehouse_id").to_pylist(),
            strict=False,
        )
    )
