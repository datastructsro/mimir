...
def fit_demand(
    ...
    Args:
        ...
        warehouse_id: Odoo warehouse ID
        points: Sorted list of DemandPoints
        forecast_horizon_days: How far ahead to project
        min_data_points: Minimum observations required
        bucket: 'daily' or 'weekly' (affects demand normalization)
...
def compute_quantile_reorder_points(
    ...
    Interpolates demand history to a gapless daily series, computes rolling
    sums over lead-time-day windows, and returns the service_level-th
    percentile as the reorder point (min_qty). Same for (lead_time + review)
    windows for max_qty.
