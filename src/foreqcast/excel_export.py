...
def export_to_excel(
    ...
    _write_review_sheet(wb, rules, forecasts)
...
def _write_review_sheet(wb: Workbook, rules: list[dict], forecasts: dict):
    ...
    for row_num, rule in enumerate(rules, 2):
        ...
        ws.cell(row=row_num, column=2, value=rule.get("product_name", ""))
        ws.cell(row=row_num, column=3, value=rule.get("warehouse_code", ""))
        ws.cell(row=row_num, column=4, value=rule.get("action", ""))
        ws.cell(row=row_num, column=5, value=rule.get("skip_reason", ""))
        ws.cell(row=row_num, column=6, value=rule.get("product_min_qty", 0))
        ws.cell(row=row_num, column=7, value=rule.get("product_max_qty", 0))
        ws.cell(row=row_num, column=8, value=rule.get("forecasted_daily_demand", 0))
        ws.cell(row=row_num, column=9, value=rule.get("lead_time_days", 0))
        ws.cell(row=row_num, column=10, value=rule.get("service_level", 0))
        ws.cell(row=row_num, column=11, value=rule.get("review_period_days", 0))

        # Inventory columns
        ws.cell(row=row_num, column=12, value=rule.get("on_hand_qty"))
        ws.cell(row=row_num, column=13, value=rule.get("reserved_qty"))
        ws.cell(row=row_num, column=14, value=rule.get("incoming_supply_qty"))
        ws.cell(row=row_num, column=15, value=rule.get("net_available_qty"))
        ws.cell(row=row_num, column=16, value=rule.get("coverage_days"))

        # Inventory flag with color
        flag = rule.get("inventory_flag", "no_data")
        flag_cell = ws.cell(row=row_num, column=17, value=flag)
        if flag in FLAG_FILLS:
            flag_cell.fill = FLAG_FILLS[flag]

        # Forecast quality from forecast.parquet
        ws.cell(row=row_num, column=18, value=fc.get("trend_slope"))
        ws.cell(row=row_num, column=19, value=fc.get("r_squared"))
        ws.cell(row=row_num, column=20, value=fc.get("confidence", ""))
        ws.cell(row=row_num, column=21, value=fc.get("data_points"))
